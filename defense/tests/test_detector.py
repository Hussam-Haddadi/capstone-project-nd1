"""Unit + regression tests for PhishingDetector."""
import sys
from pathlib import Path
sys.path.insert(0, str(Path(__file__).parent.parent.parent))

from defense.src.detector import PhishingDetector
from defense.src.features import extract_domains, is_typosquatting, is_leetspeak
from defense.src.rules import hard_rules

MODEL = str(Path(__file__).parent.parent / "models" / "defence_model.pkl")
...
def test_all():
    ...
    print(f"✓ All tests passed ({len(phishing)} phishing + {len(safe)} safe + features + rules)")
